Create Codex pet row `running-right` for `xiaowei`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Xiaowei is a timid 4-month-old golden chinchilla Persian kitten. Very fluffy long cream-golden fur with warmer golden-brown tabby shading on the head, back and plumed tail; pale cream chest, muzzle and paws. Round flat face, tiny pink nose, small ears tucked into fur, large round blue-grey eyes with a slightly worried, shy expression. Compact round chibi-friendly body. Personality: gentle, cautious, easily startled, endearing.. Preserve silhouette, face, palette, material, proportions, markings, and props.
